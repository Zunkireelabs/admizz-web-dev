...
/** Wraps preview content in a faux browser-chrome bar (macOS dots + URL pill). */
function BrowserChrome({ url, children }: { url: string; children: ReactNode }) {
  return (
    <div
      className="w-full rounded-2xl overflow-hidden h-full flex flex-col"
      style={{
        background: "#FFFFFF",
        border: "1px solid #EAECF0",
        boxShadow:
          "0 4px 24px rgba(16,24,40,0.05), 0 12px 40px rgba(16,24,40,0.04)",
      }}
    >
      <div
      ...
      >
        ...
        <span
          className="ml-2 flex-1 max-w-[180px] sm:max-w-[260px] mx-auto px-3 py-1 rounded-md text-[10.5px] font-semibold flex items-center gap-1.5 min-w-0 truncate"
          style={{
            background: "#FFFFFF",
            border: "1px solid #EAECF0",
            color: "#5C7189",
        ...
        >
          <svg className="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 11c0-1.105.895-2 2-2s2 .895 2 2-.895 2-2 2-2-.895-2-2zm0 0V7m0 4v4m-7-2a7 7 0 1114 0 7 7 0 01-14 0z" />
          </svg>
          {url}
        </span>
      </div>
      ...
      <>
        ...
        <div className="space-y-3">
          {["Full Name", "Email Address", "How will you share Admizz?"].map((f) => (
            <div key={f}>
              <p className="text-[10px] font-semibold mb-1.5" style={{ color: "#5C7189" }}>
                {f}
              </p>
          ...
          <div
            className="mt-4 h-10 rounded-lg flex items-center justify-center"
    ...
    preview: (
      <>
        ...
        <div className="grid grid-cols-2 gap-2.5 mb-3">
          {[
            ...
            <div key={s.label} className="rounded-lg p-3" style={{ background: "#F8F9FC", border: "1px solid #EAECF0" }}>
              ...
              <p className="text-sm font-extrabold mt-1" style={{ color: "#001353" }}>
                {s.val}
              </p>
            </div>
          ))}
        ...
        >
          ...
          <span className="ml-auto text-[9px] font-bold" style={{ color: "#b07400" }}>
            Copy
          </span>
        </div>
      </>
  ...
  },
  {
    ...
    preview: (
      <>
          ...
          {[
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-medium" style={{ color: "#5C7189" }}>
                  {e.event}
                </span>
              </div>
              <span className="text-[12px] font-extrabold" style={{ color: e.color }}>
                {e.amount}
              </span>
            </div>
          ))}
        ...
        >
          <span className="text-[11px] font-semibold" style={{ color: "#5C7189" }}>
            Total earned
          </span>
...
export default function HowItWorks() {
  ...
  return (
    ...
    >
      {/* Subtle pale dot grid */}
      <div
        className="absolute inset-0 pointer-events-none opacity-60"
        style={{
          backgroundImage: "radial-gradient(circle, rgba(0,19,83,0.05) 1px, transparent 1px)",
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 70% 40% at 50% 0%, rgba(252,183,48,0.05) 0%, transparent 60%)",
        }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <h2
            className="text-3xl md:text-[44px] font-extrabold leading-[1.1] tracking-[-0.015em]"
            style={{ color: "#001353" }}
          >
            From Application to Earning
            <br className="hidden sm:block" /> in Under 48 Hours
          </h2>
        ...
        {/* Steps grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-10 relative">
          {STEPS.map((step, i) => (
            <motion.div
              key={step.num}
            ...
            >
              {/* Big outlined numeral + title block */}
              <div className="flex items-start gap-5 mb-7 relative z-10">
                {/* Numeral */}
                <span
                  className="font-extrabold leading-none tracking-tight flex-shrink-0 select-none"
                  style={{
                    fontSize: "clamp(54px, 5vw, 68px)",
                    background: `linear-gradient(135deg, ${step.accent} 0%, #001353 130%)`,
                    WebkitBackgroundClip: "text",
                    WebkitTextFillColor: "transparent",
                    filter: `drop-shadow(0 6px 18px ${step.accent}25)`,
                  }}
                >
                  {step.num}
                </span>
                ...
                <div className="pt-2 flex-1 min-w-0">
                  <h3
                    className="text-[19px] md:text-[20px] font-extrabold tracking-tight mb-2"
                    style={{ color: "#001353" }}
                  >
                    {step.title}
                  </h3>
              ...
              {/* Preview wrapped in browser chrome */}
              <div className="flex-1 flex flex-col">
                <BrowserChrome url={step.url}>{step.preview}</BrowserChrome>
              </div>
            </motion.div>
          ))}
        </div>

...
const HEAR_OPTIONS = ["Student referral", "Social media", "Seminar / Event", "Direct / Google"] as const;
  ...
  return (
    <div>
      ...
      <div className="mt-2 pb-2 space-y-2">
        ...
        {/* Row 2: Seminar / Event + Direct / Google */}
        <div className="grid grid-cols-2 gap-2">
          {([
            {
              opt: "Seminar / Event" as const,
              icon: (
                <svg className="w-[18px] h-[18px] flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
export default function RegisterForm({ onStepChange, onSubmitSuccess, hideInternalSuccess }: RegisterFormProps = {}) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // URL param takes priority (strongest signal — student is on /register?ref=CODE right now).
      // Also refresh sessionStorage so the current session is properly tagged.
      // readAffiliateRefCookie() checks sessionStorage first, then the 30-day cookie —
      // this prevents stale cookies from falsely attributing organic return visits.
      ...
      const [supabaseResult] = await Promise.allSettled([
        ...
        postToCRM(form, refCode, affiliateName),
      ]);
